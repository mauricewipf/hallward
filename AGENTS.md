...
- Verify Ask AI and image editing with `cargo test --test ask_ai`, which drives full requests against a stub OpenRouter HTTP transport. Extend that suite instead of testing by hand. Q&A and edits both call OpenRouter; if no key is configured, the TUI shows setup instructions (`hallward credentials set` or `HALLWARD_OPENROUTER_API_KEY`).
