...
| d                                   | Delete the focused photo/video, or all **marked** items, after a y/n confirm. Also removes a Live Photo motion `.MOV` beside a still. Then reindexes. Works while stills are marked (Ask AI). In Search / Ask AI, `d` types as usual                                                                                       |
| c                                   | Copy the focused photo/video, or all **marked** items. Press `c` again on a copied photo to un-copy it. Newly marked items are added to an existing copy/cut set (`c` on a cut set switches it to copy). Copied thumbnails get a double border. `p` pastes. In Search / Ask AI, `c` types as usual                          |
| x                                   | Cut the focused photo/video, or all **marked** items. Press `x` again on a cut photo to un-cut it. Newly marked items are added to an existing copy/cut set (`x` on a copy set switches it to cut). Cut thumbnails get a dashed border. `p` moves them. In Search / Ask AI, `x` types as usual                              |
| p                                   | Paste copied or cut items into the current album. Newly marked items are added to the clipboard first. Same names become `photo-2.jpg`, `photo-3.jpg`, …. Live Photo motion files stay paired. Copy can paste again; cut clears after a move. In Search / Ask AI, `p` types as usual                                       |
